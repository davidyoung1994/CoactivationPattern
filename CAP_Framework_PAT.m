function CAP_Framework_PAT(working_dir,subjfolder,Data_Path,CAP_HC_path,K_Means_Para)
...
roi_signal_all = [];
for subji = 1:sub_num
    data = y_ReadAll([Data_Path.Sub 'PAT' filesep subjfolder(subji).name]);
    [~,~,~,TP_num] = size(data);
...
clust = CAP_Kmeans_corr(roi_signal_all,k_range,distance_func,CAP_HC_path);

save([working_dir 'clust.mat'],'clust')
